    ...
    (function(Reflect3) {
      (function(factory) {
        ...
        function CreateWeakMapPolyfill() {
          ...
          function GetOrCreateWeakMapTable(target, create5) {
...
var Database = class {
  ...
  async connection() {
    try {
      ...
      console.error(`Error conneting to database: ${error2}`);
      throw new Error(`Error connecting to database: ${error2}`);
    }
  }
  get clientInstance() {
    return this.client;
...
// src/use-cases/factory/make-find-person-use-case.ts
...
// src/utils/global-error-handle.ts
...
var errorHandlerMap = {
  ...
  ResourceNotFoundError: (error2, __, reply) => {
    return reply.status(404).send({ message: error2.message });
  }
};
var globalErrorHandler = (error2, _, reply) => {
...
function makeCreateAddressUseCas() {
...
async function create3(request, reply) {
  ...
  const createAddressUseCase = makeCreateAddressUseCas();
...
app.listen({
  ...
  console.log(`Servidor is running on http://localhost:${env.PORT}`);
});
